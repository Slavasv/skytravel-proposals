...
export default async function DestinationPage({ params }: { params: Promise<Params> }) {
  ...
  // Подзаголовок-маршрут для обложки: список городов из city-блоков, по порядку
  const cityNames: string[] = []
  days?.forEach((day) => {
    day.day_blocks?.forEach((db: BlockShape) => {
      if (db.content_blocks?.type === 'city' && db.content_blocks.title_ru) {
        if (!cityNames.includes(db.content_blocks.title_ru)) cityNames.push(db.content_blocks.title_ru)
      }
    })
  })
  const routeSubtitle = cityNames.join(' + ')

  // Сезон: показываем диапазон дат как иллюстративный сезон
  const seasonLabel = proposal.start_date
    ? `${formatDateRu(proposal.start_date)}${proposal.end_date ? ' — ' + formatDateRu(proposal.end_date) : ''}`
    : ''
      ...
      {proposal.cover_image_url && (
        ...
        >
          <div>
            ...
            {routeSubtitle && (
              <div style={{ fontSize: '16px', marginTop: '8px', opacity: 0.9 }}>{routeSubtitle}</div>
            )}
          </div>
        </div>
      )}
