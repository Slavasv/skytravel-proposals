  ...
  return (
    <div className="page-pad-40" style={{ padding: '40px', fontFamily: 'system-ui', maxWidth: '860px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 500, margin: '0 0 4px', letterSpacing: '-0.01em' }}>{tr(lang, 'Bookings', 'Бронирования')}</h1>
          <p style={{ color: 'var(--admin-text-muted)', margin: 0, fontSize: '14px' }}>
            {bookings.length} {bookings.length === 1 ? tr(lang, 'booking', 'бронирование') : tr(lang, 'bookings', 'бронирований')}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {isAdmin && (
            <div style={{ display: 'flex', gap: '2px', background: 'var(--admin-border-card)', borderRadius: '8px', padding: '3px' }}>
              <a href="/admin/bookings" style={{ padding: '6px 14px', fontSize: '12px', fontWeight: 500, borderRadius: '6px', textDecoration: 'none', background: !showAll ? 'var(--admin-text-on-dark)' : 'transparent', color: !showAll ? 'var(--admin-dark-panel)' : 'var(--admin-text-muted)' }}>{tr(lang, 'My', 'Мои')}</a>
              <a href="/admin/bookings?view=all" style={{ padding: '6px 14px', fontSize: '12px', fontWeight: 500, borderRadius: '6px', textDecoration: 'none', background: showAll ? 'var(--admin-text-on-dark)' : 'transparent', color: showAll ? 'var(--admin-dark-panel)' : 'var(--admin-text-muted)' }}>{tr(lang, 'All', 'Все')}</a>
            </div>
          )}
          <span style={{ fontSize: '12px', color: 'var(--admin-text-muted)' }}>
            {tr(lang, 'Bookings are created from requests', 'Бронирования создаются из запросов')}
          </span>
        </div>
      </div>
