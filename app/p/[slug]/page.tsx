    ...
    .select(`
      *,
      day_blocks (
        ...
        custom_note,
        content_blocks (
          id, type, title, description, image_url, location
        )
      )
    `)
      ...
      {days?.map((day) => {
        ...
        return (
          <div key={day.id} style={{ marginBottom: '48px' }}>
            ...
            {sortedBlocks.map((db: { content_blocks: { id: string, type: string, title: string, description: string, image_url: string, location: string }, custom_note: string | null }) => {
